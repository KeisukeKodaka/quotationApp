...
class QuotationWorkDataManager {
    
    static let sharedInstance = QuotationWorkDataManager()
    
    //名言と名前を格納する為の配列
    var quotationDataArray = [QuotationWorkData]()
    ...
    func loadQuotation() {
        ...
        guard let csvFilePath = Bundle.main.path(forResource: "work",
                                                 ofType: "csv") else {
                                                    print("csvファイルがありません")
                                                    return
        }
        ...
        do {
            ...
            csvStringData.enumerateLines(invoking: { (line, stop) in
                ...
                //問題データを格納するオブジェクト
                let quotationData = QuotationWorkData(quotationSourceDataArray: quotationSourceDataArray)
    ...
    func nextQuotation() -> QuotationWorkData {
